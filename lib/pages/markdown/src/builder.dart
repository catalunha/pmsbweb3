...
List<String> _kListTags = <String>['ul', 'ol'];
...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  Widget _buildImage(String src) {
    ...
    if (uri.scheme == 'http' || uri.scheme == 'https') {
      ...
      child = Container();// Image.file( File(filePath), width: width, height: height);
    }
  ...
  Widget _handleDataSchemeUri(Uri uri, final double width, final double height) {
    ...
    if (mimeType.startsWith('image/')) {
      return  Image.memory(uri.data.contentAsBytes(), width: width, height: height);
  ...
  Widget _buildBullet(String listTag) {
    if (listTag == 'ul'){
      return Text('•', textAlign: TextAlign.center, style: styleSheet.styles['li']);
    }

    final int index = _blocks.last.nextListIndex;
    return  Padding(
      padding: EdgeInsets.only(right: 5.0),
      child:  Text('${index + 1}.', textAlign: TextAlign.right, style: styleSheet.styles['li']),
    );
  }
